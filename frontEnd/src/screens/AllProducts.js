...
export default function AllProducts() {
 ...
 const alert = useAlert()
  ...
  useEffect(() => {
    api
      .get(
        `/api/v1/products?category=${category}&page=${currentPage}&price[gte]=${value[0]}&price[lte]=${value[1]}`
      )
      .then((res) => {
        settotalPages(
          Math.ceil(res.data.filteredProductCount / res.data.resultPerPage)
        );
    ...
    <div>
      ...
      <div className="AllProductsContainer">
        ...
        <div className="AllproductsList">
          {products.length === 0
            ? `No Products Founds with current query change price/category`
            : products.map((item) => (
